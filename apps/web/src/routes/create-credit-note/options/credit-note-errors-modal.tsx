...
const CreditNoteErrorsModal = () => {
  const creditNoteErrors = useAtomValue(creditNoteErrorAtom);
  const [open, setOpen] = useState(false);
  const [shake, setShake] = useState(false);

  const hasErrors = creditNoteErrors.length > 0;
      ...
      <DialogContent className="max-w-lg">
        <DialogHeaderContainer>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              Please fix the following errors to generate the credit note
            </DialogDescription>
          </DialogHeader>
        </DialogHeaderContainer>
        <div className="max-h-[400px] overflow-y-auto">
          <div className="flex flex-col gap-3">
            {creditNoteErrors.map((error, index) => (
              <div
                key={index}
...
export default CreditNoteErrorsModal;
