import { useParams, Navigate } from "react-router-dom";
import { CreateQuotationPage } from "./create-quotation";
import { getQuotationById } from "@/lib/indexdb-queries/quotation";
import { useQuery } from "@tanstack/react-query";
import { useQuotation } from "@/api/quotations";
import { Loader2Icon } from "@/components/ui/icons";
import type { ZodCreateQuotationSchema } from "@/zod-schemas/quotation/create-quotation";

export function EditQuotationPage() {
  const { type, id } = useParams<{ type: string; id: string }>();

  // Fetch from IndexedDB for local quotations
  const localQuotation = useQuery({
    queryKey: ["idb-quotation", id],
    queryFn: () => getQuotationById(id!),
    enabled: type === "local" && !!id,
  });

  // Fetch from server for server quotations
  const serverQuotation = useQuotation(id!, {
    enabled: type === "server" && !!id,
  });

  if (!type || !id) {
    return <Navigate to="/quotations" replace />;
  }

  // Loading state
  if (type === "local" && localQuotation.isLoading) {
    return (
      <div className="flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <Loader2Icon className="size-8 animate-spin text-muted-foreground" />
          <p className="text-sm text-muted-foreground">Loading quotation...</p>
        </div>
      </div>
    );
  }

  if (type === "server" && serverQuotation.isLoading) {
    return (
      <div className="flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <Loader2Icon className="size-8 animate-spin text-muted-foreground" />
          <p className="text-sm text-muted-foreground">Loading quotation...</p>
        </div>
      </div>
    );
  }

  // Error state
  if (type === "local" && !localQuotation.data) {
    return <Navigate to="/quotations" replace />;
  }

  if (type === "server" && !serverQuotation.data) {
    return <Navigate to="/quotations" replace />;
  }

  // Transform the data to match the form schema
  let defaultQuotation: ZodCreateQuotationSchema | undefined;

  if (type === "local" && localQuotation.data) {
    defaultQuotation = localQuotation.data.quotationFields;
  }

  if (type === "server" && serverQuotation.data?.quotationFields) {
    const fields = serverQuotation.data.quotationFields;

    // Skip if required fields are missing
    if (!fields.companyDetails || !fields.clientDetails || !fields.quotationDetails) {
      return <Navigate to="/quotations" replace />;
    }

    // Get template from server or use default
    const serverTemplate = fields.quotationDetails.theme?.template;
    const mappedTemplate = serverTemplate ?? "default";

    defaultQuotation = {
      companyDetails: {
        name: fields.companyDetails.name,
        address: fields.companyDetails.address,
        logo: fields.companyDetails.logo ?? undefined,
        signature: fields.companyDetails.signature ?? undefined,
        metadata: fields.companyDetails.metadata ?? [],
      },
      clientDetails: {
        name: fields.clientDetails.name,
        address: fields.clientDetails.address,
        metadata: fields.clientDetails.metadata ?? [],
      },
      quotationDetails: {
        theme: {
          baseColor: fields.quotationDetails.theme?.baseColor ?? "#2563EB",
          mode: (fields.quotationDetails.theme?.mode ?? "light") as "dark" | "light",
          template: mappedTemplate as "default" | "cynco" | "classic",
        },
        currency: fields.quotationDetails.currency,
        prefix: fields.quotationDetails.prefix,
        serialNumber: fields.quotationDetails.serialNumber,
        date: new Date(fields.quotationDetails.date),
        validUntil: fields.quotationDetails.validUntil ? new Date(fields.quotationDetails.validUntil) : undefined,
        paymentTerms: fields.quotationDetails.paymentTerms ?? "",
        billingDetails: (fields.quotationDetails.billingDetails ?? []).map((bd) => ({
          label: bd.label,
          value: typeof bd.value === "string" ? parseFloat(bd.value) ?? 0 : bd.value,
          type: ("type" in bd ? bd.type : "fixed") as "fixed" | "percentage",
        })),
      },
      items: fields.items.map((item) => ({
        name: item.name,
        description: item.description ?? "",
        quantity: item.quantity,
        unitPrice: typeof item.unitPrice === "string" ? parseFloat(item.unitPrice) ?? 0 : item.unitPrice,
      })),
      metadata: {
        notes: fields.metadata?.notes ?? "",
        terms: fields.metadata?.terms ?? "",
        paymentInformation: fields.metadata?.paymentInformation ?? [],
      },
    };
  }

  return <CreateQuotationPage defaultQuotation={defaultQuotation} />;
}

export default EditQuotationPage;
