import type {
  ZodCreateQuotationSchema} from "@/zod-schemas/quotation/create-quotation";
import {
...
const QuotationPreview = ({ form }: { form: UseFormReturn<ZodCreateQuotationSchema> }) => {
  ...
  useEffect(() => {
    const processFormValue = (value: ZodCreateQuotationSchema) => {
      ...
      // If the data is valid, set the data to quotation and clear the errors
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const blob = await createQuotationPdfBlob({ quotationData: data, template: form.watch("quotationDetails.theme.template") });
