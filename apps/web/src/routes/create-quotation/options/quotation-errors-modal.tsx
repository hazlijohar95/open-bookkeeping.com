...
import { quotationErrorAtom } from "@/global/atoms/quotation-atom";
...
const QuotationErrorsModal = () => {
  const quotationErrors = useAtomValue(quotationErrorAtom);
  const [open, setOpen] = useState(false);
  const [shake, setShake] = useState(false);

  const hasErrors = quotationErrors.length > 0;
      ...
      <DialogContent className="max-w-lg">
        <DialogHeaderContainer>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              Please fix the following errors to generate the quotation
            </DialogDescription>
          </DialogHeader>
        </DialogHeaderContainer>
        <div className="max-h-[400px] overflow-y-auto">
          <div className="flex flex-col gap-3">
            {quotationErrors.map((error, index) => (
              <div
                key={index}
...
export default QuotationErrorsModal;
