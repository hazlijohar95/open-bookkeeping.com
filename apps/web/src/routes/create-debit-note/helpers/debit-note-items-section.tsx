...
interface DebitNoteItemsSectionProps {
  form: UseFormReturn<ZodCreateDebitNoteSchema>;
}
type DebitNoteItem = ZodCreateDebitNoteSchema["items"][number];

const DebitNoteItemsSection: React.FC<DebitNoteItemsSectionProps> = ({ form }) => {
              ...
              <div className="flex w-full flex-row gap-2">
                ...
                <div className="w-full">
                  ...
                  <div className="text-primary text-[10px] font-medium">
                    {formatCurrencyText(form.watch("debitNoteDetails.currency"), field.unitPrice)}{" "}
                ...
                <div className="flex flex-col items-end justify-between gap-1">
                  ...
                  <div className="flex flex-row items-center gap-1">
                    <p className="space-x-1 text-[10px] whitespace-nowrap">
                      <span>Total:</span>
                      <span>
                        {formatCurrencyText(form.watch("debitNoteDetails.currency"), field.unitPrice * field.quantity)}
                      </span>
                    </p>
                  </div>
                </div>
...
export default DebitNoteItemsSection;
...
interface AddItemModalProps {
  ...
  append: (data: DebitNoteItem) => void;
  update: (index: number, data: DebitNoteItem) => void;
}
...
const HandleItemModal = ({ type, append, update, editingIndex, data, children }: AddItemModalProps) => {
  ...
  const onHandleSubmit = (data: DebitNoteItem) => {
    if (type === "edit" && typeof editingIndex === "number") {
      update(editingIndex, data);
    } else {
      append(data);
    }
    debitNoteItemForm.reset();
    setOpen(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <Form {...debitNoteItemForm}>
          <form onSubmit={debitNoteItemForm.handleSubmit(onHandleSubmit)}>
            <DialogHeaderContainer>
              <DialogIcon>
                {type === "edit" ? <FilePenIcon /> : <BoxPlusIcon />}
              </DialogIcon>
              <DialogHeader>
                <DialogTitle>{type === "add" ? "Add Debit Item" : "Edit Item"}</DialogTitle>
                <DialogDescription>
                  {type === "add" ? "Add a debit item to your note" : "Update the item details below"}
                </DialogDescription>
              </DialogHeader>
            </DialogHeaderContainer>
            ...
            <DialogContentContainer>
              <FormInput
                label="Name"
                name="name"
                placeholder="e.g. Additional Service, Late Fee"
                reactform={debitNoteItemForm}
              />
              <FormInput
                label="Description"
                name="description"
                placeholder="e.g. Rush delivery charge"
                reactform={debitNoteItemForm}
                isOptional={true}
              />
