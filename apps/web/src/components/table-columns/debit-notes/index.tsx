...
const columnConfigHelper = createColumnConfigHelper<DebitNoteApiResponse>();

// Helper to calculate total from items
const getTotalValue = (fields: DebitNoteApiResponse["debitNoteFields"]) => {
...
export const columns = [
  ...
  columnHelper.accessor((row) => row.debitNoteFields?.clientDetails?.name, {
    id: "client",
  ...
  columnHelper.accessor((row) => getTotalValue(row.debitNoteFields), {
    id: "total",
  ...
  columnHelper.accessor(() => "actions", {
    ...
    cell: ({ row }) => {
      ...
      const updateStatusMutation = trpc.debitNote.updateStatus.useMutation({
        onSuccess: () => utils.debitNote.list.invalidate(),
      });
...
const getStatusBadgeVariant = (status: DebitNoteStatusType): BadgeVariants => {
  switch (status) {
    case "draft":
      return "secondary";
    case "issued":
...
const getStatusIcon = (status: DebitNoteStatusType) => {
  switch (status) {
    case "draft":
