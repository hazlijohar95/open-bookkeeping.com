...
const DebitNoteDefaultPDF: React.FC<{ data: ZodCreateDebitNoteSchema }> = ({ data }) => {
  const isDark = data.debitNoteDetails.theme.mode === "dark";
  const accent = data.debitNoteDetails.theme.baseColor;
  const subtotal = getDebitNoteSubTotalValue(data);
  const total = getDebitNoteTotalValue(data);
  ...
  return (
    <Document
      title={`Debit Note ${data.debitNoteDetails.prefix}${data.debitNoteDetails.serialNumber}`}
      ...
      <Page size="A4" style={{ ...tw(`p-12 ${surface} ${ink}`), fontFamily: "Geist" }}>
        ...
        <View style={tw("flex flex-row justify-between items-start mb-10")}>
          ...
          <View style={tw("flex flex-col")}>
            ...
            <Text style={tw(`text-[32px] font-semibold tracking-tight leading-none ${isDark ? "text-dark-ink" : `text-[${accent}]`}`)}>
              {data.debitNoteDetails.prefix}{data.debitNoteDetails.serialNumber}
            </Text>
          </View>
        ...
        <View style={tw(`flex flex-row gap-8 pb-6 mb-8 border-b ${border}`)}>
          <View style={tw("flex flex-col gap-1")}>
            ...
            <Text style={tw(`text-[11px] font-medium ${ink}`)}>{format(data.debitNoteDetails.date, "MMM dd, yyyy")}</Text>
          </View>
          {data.debitNoteDetails.originalInvoiceNumber && (
            <View style={tw("flex flex-col gap-1")}>
              <Text style={tw(`text-[9px] font-medium tracking-wider uppercase ${inkFaint}`)}>Original Invoice</Text>
              <Text style={{ ...tw(`text-[11px] font-medium ${ink}`), ...mono }}>{data.debitNoteDetails.originalInvoiceNumber}</Text>
            </View>
          )}
          <View style={tw("flex flex-col gap-1")}>
            <Text style={tw(`text-[9px] font-medium tracking-wider uppercase ${inkFaint}`)}>Currency</Text>
            <Text style={{ ...tw(`text-[11px] font-medium ${ink}`), ...mono }}>{data.debitNoteDetails.currency}</Text>
          </View>
        </View>
        ...
        <View style={tw("flex flex-row gap-6 mb-8")}>
          ...
          {/* To */}
          <View style={tw(`flex flex-col flex-1 p-5 rounded-lg ${surfaceAlt}`)}>
            <Text style={tw(`text-[9px] font-semibold tracking-[0.15em] uppercase ${inkFaint} mb-3`)}>Debit To</Text>
            ...
            {data.items.map((item, i) => (
              <View
                ...
                <Text style={{ ...tw(`w-24 text-[11px] text-right ${inkMuted}`), ...mono }}>
                  {formatCurrencyText(data.debitNoteDetails.currency, item.unitPrice)}
                </Text>
                <Text style={{ ...tw(`w-24 text-[11px] font-medium text-right ${ink}`), ...mono }}>
                  {formatCurrencyText(data.debitNoteDetails.currency, item.quantity * item.unitPrice)}
                </Text>
              </View>
            ))}
          </View>
            ...
            <View style={tw(`p-4 rounded-lg ${surfaceAlt}`)}>
              <View style={tw("flex flex-row justify-between items-center")}>
                ...
                <Text style={{ ...tw(`text-[10px] ${ink}`), ...mono }}>
                  {formatCurrencyText(data.debitNoteDetails.currency, subtotal)}
                </Text>
              </View>

              {data.debitNoteDetails.billingDetails.map((detail, i) => (
                <View key={i} style={tw("flex flex-row justify-between items-center mt-1.5")}>
                  <Text style={tw(`text-[10px] ${inkMuted}`)}>{detail.label}</Text>
                  <Text style={{ ...tw(`text-[10px] ${ink}`), ...mono }}>
                    {detail.type === "percentage" ? `${detail.value}%` : formatCurrencyText(data.debitNoteDetails.currency, detail.value)}
                  </Text>
                </View>
              ))}
              ...
              <View style={tw(`border-t ${border} mt-3 pt-3`)}>
                <View style={tw("flex flex-row justify-between items-center")}>
                  <Text style={tw(`text-[11px] font-semibold ${ink}`)}>Total Due</Text>
                  <Text style={{ ...tw(`text-[18px] font-semibold tracking-tight ${isDark ? "text-dark-ink" : `text-[${accent}]`}`), ...mono }}>
                    {formatCurrencyText(data.debitNoteDetails.currency, total)}
                  </Text>
                </View>
              </View>
...
export default DebitNoteDefaultPDF;
