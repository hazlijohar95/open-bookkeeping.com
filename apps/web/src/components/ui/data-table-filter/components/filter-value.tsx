...
export function FilterValueOptionDisplay<TData>({ filter, column }: FilterValueDisplayProps<TData, "option">) {
  ...
  if (selected.length === 1) {
    ...
    const { label, icon: Icon } = firstSelected;
    const hasIcon = !!Icon;
    return (
...
export function FilterValueNumberController<TData>({
  ...
  const changeType = useCallback(
    (type: "single" | "range") => {
      ...
      if (type === "single")
        newValues = [val0]; // Keep the first value for single mode
      else if (!minMax) newValues = createNumberRange([val0, val1]);
      else {
        newValues =
          val0 - minMax[0] < minMax[1] - val0
            ? createNumberRange([val0, minMax[1]])
            : createNumberRange([minMax[0], val0]);
      }
              ...
              <TabsContent value="range" className="mt-4 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <DebouncedInput type="number" value={values[0] ?? 0} onChange={(v) => changeMinNumber(Number(v))} />
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium">{t("max", locale)}</span>
                    <DebouncedInput type="number" value={values[1] ?? 0} onChange={(v) => changeMaxNumber(Number(v))} />
                  </div>
                </div>
              </TabsContent>
            </Tabs>
